...
## Early support for PostgresSQL
Setting `DOLI_DB_TYPE` to `pgsql` enable Dolibarr to run with a PostgresSQL database.
...
| **DOLI_ADMIN_LOGIN**          | *admin*                        | Administrator's login create on the first boot |
| **DOLI_ADMIN_PASSWORD**       | *admin*                        | Administrator's password |
